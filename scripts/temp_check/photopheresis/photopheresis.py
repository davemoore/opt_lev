...
def Intensity_to_loose_constant_grad_extra_heat(p, Rgas, r, eta, Tgas, MolecularMass, constant, acomo, c1, c2, c3): # the gradient is constant in pressure and is prop to co2 power
    ...
    a = ( 1.*constant - 1.0*(gamma0*c1 + c2*c3)/(gamma0 + c3) )*(1.*p/p0 + 1.*p0/p)
...
#para = [8.31, 7.5e-06, 1.8e-05, 300.0, 0.029, 0.9978998960849689, 6.431133751537939, 0.997899895677771, 0.997899896814544, 1.9933715365252267]
print(para)
para2 = [8.31, 7.5e-06, 1.8e-05, 300.0, 0.029, 1e-2*4.630756456833827e-12, -0.006130073603417986, 10.] + [-1e-13]
plt.figure()
