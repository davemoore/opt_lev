...
t = open("thermal_model/temp_data_emissiv0_1_multpow.txt", 'rt')
l = t.readlines()
t.close()
...
## now get the data
d = np.loadtxt( "thermal_model/temp_data_emissiv0_1_multpow.txt", skiprows=9 )
...
for pow_col in [0.1, 1, 10]:
    ...
    for cpt in np.argwhere(gpts):
        ...
        tgrad = (np.interp( -r, xvals.T, cdat) - np.interp( r, xvals.T, cdat))/(2*r)
    ...
    plt.semilogx( tvp[:-1,0], tvp[:-1,1], label="%g $\mu$W/$\mu$m$^2$"%(0.1*pow_col))

    plt.figure(fig2.number)
    plt.loglog( tvp[:-1,0], tvp[:-1,2], label="%g $\mu$W/$\mu$m$^2$"%(0.1*pow_col))
...
plt.legend()
plt.xlabel('Pressure [mbar]')
plt.ylabel('Temperature gradient [K/$\mu$m]')
plt.ylim([0,2])
