...
def get_dc_volt(s):
    ...
    return float(re.findall(elec_str+"_\d+mV")[0][5:-2])

def get_z_pos(s):
    return float( re.findall("Z\d+nm")[0][1:-2] )

def sort_fun( s):
    # sort first by the cantilever position, then sort by 
    # the voltage on the cantilever
    return get_z_pos(s)*1e6 + get_dc_volts(s)

def get_stage_elec_lists( flist ):
    stage_pos_list = []
    elec_volt_list = []
    for f in flist:
        elec_volt_list.append( get_dc_volts(f) )
...
volts_vs_pos = scipy.interpolate.RectBivariateSpline(xx, zz, yy)

volt_arr = volts_vs_pos(stage_arr_m, chame_force)
