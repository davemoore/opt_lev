...
do_poly_fit = False #True  ## fit data to 1/r^2 (for DC bias data)
...
def process_files(data_dir, num_files, dc_val=-999999, pos_at_10V=0., 
    ...
    if( dc_val > -999999 ):
        print "Data with DC Offset (V): ", dc_val
    ...
    for fidx,f in enumerate(flist[:num_files]):
        ...
        if(do_mean_subtract):
            bm,am = signal.butter(3,drive_freq- 5.)/(Fs/2.), btype='highpass')
            #bm,am = signal.butter(3,[(drive_freq*4 -10.)/(Fs/2.),(drive_freq*4. + 10)/(Fs/2.)], btype='bandpass')
...
def get_dc_offset_orig(s):  
    dc = re.findall("Hz\d+mVdc",s)
    curr_str = int(dc[0][2:-4])
    return curr_str
...
for cdir in dirs:
    ...
    for cf in clist:
        dc = get_dc_offset( cf )
        dc_orig = get_dc_offset_orig( cf )
        dc_list.append( dc  )
        dc_list_orig.append( dc_orig  )
    dc_list, idx = np.unique(dc_list, return_index=True)
    dc_list_orig = np.array(dc_list_orig)[idx]
    print "List of DC Vals: ", dc_list
    ...
    if(not file_exists):
        ...
        for dc_val,dc_val_orig in zip(dc_list,dc_list_orig):
            ...
            clab = str(dc_val / 1000.) + ' V'

            curr_dict['label'] = clab
            curr_dict['dc_val'] = dc_val
    ...
    for j,v in enumerate(dirs_to_plot):
        ...
        if( sep_forward_backward ):
            ...
            if( len(tot_dc_list) > 1 ): 
                coll = colors_step[ np.argwhere( tot_dc_list == curr_dc)[0] ]
            else:
                coll = colors_yeay[i]
            ...
            if not 'bins' in dataout_dict:
                dataout_dict['bins'] = bins[gpts]

            dataout_dict[label + v] = dat[gpts]
...
pickle.dump(dataout_dict, open("testout.p", "wb"))
print "wheee"
