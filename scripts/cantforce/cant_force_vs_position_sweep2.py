...
# Select channel with multiple amplitudes of sweep
do_x_sweep = True #True
...
def process_files(data_dir, num_files, sweep_amp=-999999, pos_at_10V=0., 
                  monmin=0., monmax=10., conv_fac =1., drive_indx=19):

    out_dict = {}

    if( sweep_amp > -999999 ):
        print "Data with alt sweep amplitude (nm): ", sweep_amp
        sub_name = '*' + sweep_channel + 'drive%dnm*.h5' % sweep_amp
        flist = sorted(glob.glob(os.path.join(data_dir, sub_name)), key = sort_fun)
        flist1 = []
    else:
...
def get_sweep_amp_orig(s):
    ctrl = sweep_channel + 'drive-?\d+'
    sweep_amp = re.findall(ctrl, s)[0]
    if '-' in sweep_amp:
        curr_str = -1 * int(sweep_amp.split('-')[1])
    else:
        curr_str = int(sweep_amp.split('e')[1])
    return curr_str
...
for cdir in dirs:
    ...
    amp_list_orig = []
    for cf in clist:
        amp = get_sweep_amp( cf )
        amp_orig = get_sweep_amp_orig( cf )
        amp_list.append( amp  )
        amp_list_orig.append( amp_orig  )
    amp_list, idx = np.unique(amp_list, return_index=True)
    amp_list_orig = np.array(amp_list_orig)[idx]
    print "List of secondary sweep amplitudes: ", amp_list


    if(not file_exists):

        curr_data = []
        for sweep_amp,sweep_amp_orig in zip(amp_list,amp_list_orig):
            print sweep_amp

            curr_dict = process_files(cdir[0],max_files,drive_indx=cdir[2], \
                                      sweep_amp=sweep_amp_orig, \
                                      pos_at_10V=cdir[6],conv_fac=cdir[7])


            clab = str(sweep_amp / 1000.) + ' um'
            
            curr_dict['label'] = clab
            curr_dict['sweep_amp'] = sweep_amp
...
tot_amp_list = []
for d in data:
    tot_amp_list.append( d['sweep_amp'] )
tot_amp_list = np.unique(tot_amp_list)

## make color list for given number of files
colors_yeay = bu.get_color_map( len(data) )
colors_amp = bu.get_color_map( len(tot_amp_list) )
...
for i,d in enumerate(data):
    ...
    for j,v in enumerate(dirs_to_plot):
        ...
        if( sep_forward_backward ):
            ...
            is_first_pos = True
            offset = 0.
            curr_amp = d['sweep_amp']

            gpts = dat != 0
            if( not match_overlap_region ):
                offset = -np.mean( dat[gpts] )
            if( len(tot_amp_list) > 1 ): 
                coll = colors_amp[ np.argwhere( tot_amp_list == curr_amp)[0] ]
            else:
                coll = colors_yeay[i]
...
for i,d in enumerate(data):
    ...
    step_pos = d['sweep_amp']
    #if(dc_volt != 1.0): continue
...
plt.xlabel('Position Along Cantilever Face, X-Coord [um]')
plt.legend(numpoints=1,loc=3,prop={'size':10})
plt.show()
