...
def process_files(data_dir, num_files, dc_val=-999999, pos_at_10V=0., 
    ...
    if( dc_val > -999999 ):
        ...
        if(do_y_sweep):
            flist = sorted(glob.glob(os.path.join(data_dir, "*nmY%d*.h5"%abs(dc_val))), key = sort_fun)
        if(do_z_sweep):
            flist = sorted(glob.glob(os.path.join(data_dir, "*nmZ%d*.h5"%abs(dc_val))), key = sort_fun)
        if( len( flist ) == 0 ):
...
def get_dc_offset_orig(s):  
    ...
    if( len(dcstr) == 0 and not (do_x_sweep or do_y_sweep or do_z_sweep) ):
        return -999999
    else:
        if( len(dcstr) != 0):
