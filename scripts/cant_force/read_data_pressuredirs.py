...
def proc_dir(d):
    ...
    dir_obj.diagonalize_files(simpleDCmat=True)

    dir_obj.get_conv_facs()

    pressures = []
    for fobj in dir_obj.fobjs:
        pressures.append(fobj.pressures[2])
    mean = np.mean(pressures)
    std = np.std(pressures)
    dir_obj.label = '%0.4f+%0.5f' % (mean, std)
...
for i, obj in enumerate(dir_objs):
    ...
    for key in keys:
        ...
        offset = -1.0 * obj.avg_diag_force_v_pos[key][1][-1]
        plt.figure(2)
