...
def tot_fun(x, Afix, A0, beta):
    ...
    cf = bu.get_yukawa_vs_alpha_lam(x*1e-6, beta, lam)
...
for lam in lam_list:
    ...
    down_fac = 1.0
    maxalpha = 5*1e-16/bu.get_yukawa_vs_alpha_lam(20e-6, 1, lam)    
    beta_list = np.linspace(0,maxalpha,20)
    ...
    plt.savefig("plots_yuk/bead_data_%d.pdf"%(lam*1e6))
    ...
    plt.ylim([0,5])
    plt.savefig("plots_yuk/profiles_%d.pdf"%(lam*1e6))
    ...
    UL_list.append([lam, cl90])

UL_list = np.array(UL_list)
np.save("plots_yuk/final_yuk_limits_paper.npy", UL_list)
for i in range( np.shape(UL_list)[0] ):
    print "Lambda = %d:  UL=%e" % (UL_list[i,0]*1e6, UL_list[i,1])
