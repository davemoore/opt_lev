...
for col,d,acv in zip(col_list,dlist,aclist):
    ...
    ## i.e., 4.8 x 10^-16 N.  This is just taken by applying this
    ## same code to the single e steps in the calibration data and
    ## estimating the field.  Probably only good to a factor of 2
...
plt.figure()
plt.plot(dvals, slope_list[:,1], 'ko')
plt.xlabel("Distance from bead [$\mu$m]")
