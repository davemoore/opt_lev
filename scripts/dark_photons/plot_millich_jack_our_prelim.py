...
dtype=(1,0.5)
ax.add_patch(Rectangle((1.5e-5, 1.0/3.7e20), 1.0-1.5e-5, 2.5, ec=ec, fc=prev_col, lw=2, ls=':'))
epsmin = 1.5e-5
nmin = 1.0/3.7e20
xx = np.linspace(1e-11, epsmin, 1e2)
yy = nmin * np.max(xx)/xx
...
prev_col = [0.7,0.4,0.4]
ec=[0.5,0.,0]

min_sens = 1.1e-6
min_nuc = 1./6e15 * 0.1
dtype=(1,0.5)
ax.add_patch(Rectangle((min_sens, min_nuc), 1.0-min_sens, 2.5, ec=ec, fc=prev_col, lw=2, ls=":"))
epsmin = min_sens
nmin = min_nuc
xx = np.linspace(1e-11, epsmin, 1e2)
yy = nmin * np.max(xx)/xx
