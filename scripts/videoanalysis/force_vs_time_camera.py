...
idx_to_plot = 154 ## index from dir file below to use
...
def find_flash( vdict ):
    ...
    thresh = np.mean(tot_amp) + 5.*np.std(tot_amp)
...
if( plot_fft_image ):
    ...
    for idx in range(0,np.shape(fft_image)[1],10):
        plt.loglog( fft_freqs, np.abs( fft_image[100,idx,:].flatten() ) )
    plt.show()
    ...
    f3=make_image_plot(z2, 
                       "Bead response, off drive, %f Hz"%(drive_freq+offset_freq[0]))
