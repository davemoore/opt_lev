...
lFinel  =  grid*((aspect-1)*(L+2*offset)/L+1);
lFiner  =  grid*((aspect-1)*(L-2*offset)/L+1);
...
Point(13) = { -H/2+offset, -H/2,  -D, lCoarsel};
Point(14) = { -H/2+offset, H/2,  -D, lCoarsel};
Point(15) = { H/2+offset, -H/2,  -D, lCoarser};
Point(16) = { H/2+offset, H/2,  -D, lCoarser};
Line(1) = {10, 12};
Line(2) = {12, 11};
Line(3) = {11, 9};
Line(4) = {9, 10};
